...
def _split_on_special_tokens(text: str, special_tokens: List[str]) -> List[str]:
    ...
    parts = builtin_re.split(f"({pattern})", text)  # keep separators
...
def process_chunk(args):
    ...
    for part in text_parts:
        ...
        for pretoken in pretokens:
            # IMPORTANT: store a tuple of *bytes*, not ints
            # Each element is a single-byte `bytes` object.
            b = pretoken.encode("utf-8")
            byte_tuple = tuple(bytes([bb]) for bb in b)
...
def train_bpe_parallel(
    ...
    # Find chunk boundaries based on special tokens (on bytes)
    ...
    # Read chunks in binary and decode per chunk (so byte offsets match)
    chunks = []
    with open(input_path, 'rb') as f:
    ...
    for merge_idx in range(num_merges):
        ...
        # pair_counts items: ((bytes, bytes), count)
        most_frequent_pair = max(pair_counts.items(), key=lambda x: (x[1], x[0]))[0]

        # Record this merge
        merges.append(most_frequent_pair)

        # Create new merged token (bytes + bytes = bytes)
    ...
    print(f"Total training time (pretok + merges): {total_time:.2f} seconds")
    print(f"Final vocab size: {len(vocab)}")

    # Sanity checks
    assert all(isinstance(v, (bytes, bytearray)) for v in vocab.values()), \
        "Vocab must map ids -> bytes"
    assert all(isinstance(a, (bytes, bytearray)) and isinstance(b, (bytes, bytearray)) for a, b in merges), \
        "Merges must be a list of (bytes, bytes)"

    return vocab, merges


# ----------------------
# Pair counting / merging
# ----------------------
def _count_pairs(word_freqs: Dict[Tuple[bytes, ...], int]) -> Counter:
    ...
    for word, freq in word_freqs.items():
        ...
        for i in range(len(word) - 1):
            pair = (word[i], word[i + 1])  # both are bytes
...
def analyze_vocabulary(vocab: Dict[int, bytes]) -> None:
    ...
    # Filter to bytes only (defensive)
    values = [v for v in vocab.values() if isinstance(v, (bytes, bytearray))]
    if not values:
        print("No byte tokens in vocab; something is wrong.")
        return

    # Find longest token
    longest_token = max(values, key=len)
    ...
    length_dist = Counter(len(token) for token in values)
    print(f"\nToken length distribution:")
    for length in sorted(length_dist.keys()):
        print(f"  Length {length:2d}: {length_dist[length]:5d} tokens")

    # Show some example long tokens
    long_tokens = [(token_id, token) for token_id, token in vocab.items()
                   if isinstance(token, (bytes, bytearray)) and len(token) >= 10]
    ...
    for i, (token_id, token) in enumerate(long_tokens[:10]):
        try:
            ...
            print(f"  {i+1:2d}. ID {token_id:4d}: {len(token):2d} bytes - {token!r}")
