...
# Try to use the third-party 'regex' lib (supports \p{}), else fall back to stdlib 're'
try:
...
# If 'regex' is available we can use \p{L}/\p{N}; otherwise use a close stdlib approximation.
if _HAS_REGEX:
    PAT = r"""'(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+"""
else:
    # Rough approximation without Unicode properties
    PAT = r"""'(?:[sdmt]|ll|ve|re)| ?[A-Za-z]+| ?\d+| ?[^\sA-Za-z0-9]+|\s+(?!\S)|\s+"""
